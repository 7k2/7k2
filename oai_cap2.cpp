...
int Nation::think_capture_new_enemy_town(Town* capturerTown, int useAllCamp)
{
	...
	{
		return start_capture( targetTown->town_recno, 2 );		// call AI functions in OAI_CAPT.CPP to capture the town, false-don't hire a new capturer, use an existing unit as the capturer
	}
...
void Nation::think_capturing_enemy_town()
{
	...
	if( hasWar != 2 )
	{
		...
		{
			start_capture( targetTown->town_recno, 2 );		// call AI functions in OAI_CAPT.CPP to capture the town
		}
...
int Nation::mobile_defense_combat_level(int targetXLoc, int targetYLoc, int targetNationRecno, int returnIfWar, int& hasWar)
{
	...
	int attackerPower = world.net_attacker_power_in_area(targetXLoc, targetYLoc,
							  nation_recno, targetNationRecno, returnIfWar, hasWar, scanRange);

	if( attackerPower == -1 )
		return -1;
	else
		return -attackerPower;		// negative of net attacker power is net defender power
}
