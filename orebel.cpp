...
/*
	...
	if( action_mode == REBEL_ATTACK_TOWN )
	{
		...
		{
			all_rebel_attack( town_array[action_para]->base_obj_recno );
		}
	}
	...
	else if( action_mode == REBEL_ATTACK_FIRM )
	{
		...
		{
			all_rebel_attack( firm_array[action_para]->base_obj_recno );
		}
	}
*/
}
	...
	switch( rebelPtr->action_mode )
	{
		...
		case REBEL_SETTLE_NEW:
			if( !world.can_build_town(rebelPtr->action_para, rebelPtr->action_para2, sprite_recno) )
			{
				...
				if( locPtr->is_town() &&
					 town_array[locPtr->town_recno()]->race_id == race_id &&
					 town_array[locPtr->town_recno()]->rebel_recno == rebelPtr->rebel_recno &&
					 town_array[locPtr->town_recno()]->population < MAX_POPULATION )
