...
int FirmInn::think_hire_general()
{
	...
	for( int i=0 ; i<inn_unit_count ; i++, innUnit++ )
	{
		if( unit_res[innUnit->unit_id]->is_civilian )		// only military unit, but can be either a spy general or a normal general
			continue;
...
int FirmInn::think_assign_general_to(int raceId, int innUnitRecno)
{
	...
	for( int i=ownNation->ai_camp_count-1 ; i>=0 ; i-- )
	{
		...
		if( curRating > bestRating )
		{
			...
			if( ownNation->is_action_exist(firmCamp->loc_x1, firmCamp->loc_y1,
				 -1, -1, ACTION_AI_ASSIGN_OVERSEER, firmCamp->firm_id) )		// it can be a Camp or a Fort, so we use firmCamp->firm_id
			{
				continue;
			}
	...
	ownNation->add_action(bestCamp->loc_x1, bestCamp->loc_y1, -1, -1, ACTION_AI_ASSIGN_OVERSEER, bestCamp->firm_id, 1, unitRecno);
