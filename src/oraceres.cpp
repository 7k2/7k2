...
void RaceRes::load_race_info()
{
	...
	for( i=0 ; i<race_count ; i++ )
	{
		...
		for( unitId=1 ; unitId<=MAX_UNIT_TYPE ; unitId++ )
		{
			...
			{
				if( !raceInfo->civilian_unit_id ) // assume first unit of that race is civilian
				{
					raceInfo->civilian_unit_id = unitId;
					unit_res[unitId]->is_civilian = 1;
				}
				else if( !raceInfo->infantry_unit_id ) // assume second unit of that race is infantry
				{
					raceInfo->infantry_unit_id = unitId;
				}
				else if( !raceInfo->special_unit_id ) // assume first unit of that race is cavalry
				{
...
{
	err_if( raceId<0 || raceId>race_count )
