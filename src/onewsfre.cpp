...
int News::is_major()
{
	...
	{
		//-- only rebellions happening in the player's citizens are considered as major news --//
...
void News::migrate()
{
	...
	if( nation_array.player_recno &&
		 nation_recno1 == nation_array.player_recno )		// from player nation to another nation
	{
...
void News::nation_surrender()
{
	...
	if( nation_array.player_recno &&
		 nation_recno2 == nation_array.player_recno )
	{
		str += "à vous.";
	}
	else
...
char* News::nation_name1()
{
	...
	if( nation_recno1 < 0 )		// human player - custom name
		str += nation_array.get_custom_name(nation_recno1, 1);		// 1-first word of the name only
	else
...
char* News::nation_name2()
{
	...
	if( nation_recno2 < 0 )		// human player - custom name
		str += nation_array.get_custom_name(nation_recno2, 1);		// 1-first word of the name only
	else
