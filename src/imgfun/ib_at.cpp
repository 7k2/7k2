...
void IMGbltWAreaTrans(short *imageBuf, int pitch, int desX, int desY, short *bitmapBuf,
	int srcX1, int srcY1, int srcX2, int srcY2)
{
	...
	int esi = 2 + srcY1 * bitmapWidth + srcX1;	// 4 bytes are header fields (width, height)

	for ( int j=0; j<height; ++j,destline+=pitch/2,esi+=bitmapWidth )
	{
		for ( int i=0; i<width; ++i )
		{
			if ( ((unsigned short*)bitmapBuf)[esi + i] != transparent_code_w)
			{
