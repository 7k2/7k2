...
void IMGbltBlendAreaRemapHMirror(short *imageBuf, int pitch, int desX, int desY, char *bitmapPtr,
	int srcX1, int srcY1, int srcX2, int srcY2, short *colorTable)
{
	...
	int srcline = 2 + srcY1 * bitmapWidth + srcX1		// skip 4 byte header
		      + bitmapWidth - 1; // srcline points to the end pixel of the line
	int width = srcX2-srcX1+1;
	int height = srcY2-srcY1+1;
	short ax;
	short dx;

	for ( int j=0; j<height; ++j, destline+=pitch/2, srcline+=bitmapWidth )
	{
		for ( int i=0; i<width; ++i )
		{
			ax = ((short*)bitmapPtr)[ srcline - i ];
